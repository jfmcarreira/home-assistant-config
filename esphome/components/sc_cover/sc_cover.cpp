...
void SingleControlCover::dump_config() {
  ...
  LOG_BUTTON(" ", "Activation button", this->activation_button_);
...
float SingleControlCover::get_setup_priority() const {
  return setup_priority::WIFI;
}

void SingleControlCover::setup() {
  this->open_endstop_->add_on_state_callback(
      [this](bool state) { this->open_endstop_callback_(state); });
  this->close_endstop_->add_on_state_callback(
      [this](bool state) { this->close_endstop_callback_(state); });
...
void SingleControlCover::control(const CoverCall &call) {
  ...
  if (call.get_position().has_value()) {
    ...
    if (pos != this->position) {
      ...
      this->target_operation_ =
          pos < this->position ? TARGET_OPERATION_CLOSE : TARGET_OPERATION_OPEN;
...
void SingleControlCover::loop() {
  ...
  else if ((this->target_operation_ != TARGET_OPERATION_NONE) &&
           !this->is_operation_done_()) {
...
bool SingleControlCover::is_operation_done_() const {
  return static_cast<uint8_t>(this->target_operation_) ==
         static_cast<uint8_t>(this->current_operation);
}
...
bool SingleControlCover::activate_switch_() {
  ...
  if ((now - this->last_activation_time_) > this->switch_interval_) {
    ...
    // this->activation_button_->turn_on();
    this->activation_button_->press();
